...
## 10. v3 Remediation Status (2026-09-04)

The following status applies only to the EWP/v3-only working tree. It does not
retroactively approve legacy revisions or replace an independent deployment
review.

| Date | Finding | State | Fix reference | Verification | Residual risk |
|---|---|---|---|---|---|
| 2026-09-04 | C-01 | Mitigated in v3-only tree | Legacy handshake sources and tests removed; v3 boundary test added | `TestV3SourceBoundaryHasNoEarlierHandshake`, `go test ./...` | This remains a breaking cutover; legacy code must not be reintroduced into the production build. |
| 2026-09-04 | H-01 | Fixed for v3 records | `record.go`, `padding_policy.go`, `EWP_V3_RECORDS.md` | `TestV3RecordRejectsLengthCounterAndTrailingMutations`, record KAT | Coarse record length and traffic timing remain observable. |
| 2026-09-04 | H-02 | Mitigated for v3 | `handshake_context.go`, `v3_admission.go`, `v3_api.go`, `transport.go` | cancellation, bounded-message, lease, shuffle, and race tests | Custom transports still must honor the Close-interrupt contract. |
| 2026-09-04 | H-03 | Fixed for v3 handshake order | `v3_handshake.go`, `v3_admission.go`, `v3_prekey.go` | invalid-cookie/admission tests prove no claim or KEM acquisition | Availability remains dependent on configured limits and outer network filtering. |
| 2026-09-04 | H-04 | Scoped to one running process in v3 | `PreKeyProvider.ClaimAndBurn`, `MemoryPreKeyProvider`, connection-local Finished state | prekey/replay single-use and Finished-gate tests | Replay and in-flight state are intentionally lost on process restart; the protocol does not claim deployment-wide replay protection. |
| 2026-09-04 | H-05 | Fixed for v3 prekey path | `v3_prekey.go`, `v3_handshake.go` | signed bundle, single-use burn, destruction, generation, and bundle-wire tests | Go and runtime-managed crypto object erasure remains best effort. |
| 2026-09-04 | M-01 | Mitigated for v3 | `record.go`, `securestream.go`, `v3_handshake.go`, `v3_kdf.go` | close/send-failure/key-update and race tests | Key material cannot be guaranteed erased from all Go-managed memory. |
| 2026-09-04 | M-03 | Fixed for v3 key schedule | `v3_kdf.go`, `v3_messages.go`, `v3_rekey_test.go` | fixed KATs, transcript mutation coverage, and directional rekey tests | Interoperability with an independent implementation is still required. |
| 2026-09-04 | M-04 | Length mitigated; shaping optional for v3 | `padding_policy.go`, `traffic_shaper.go`, `record.go` | opaque record KAT and benchmark/fuzz gates | Record padding hides exact inner lengths probabilistically; `StreamShaper` is opt-in and neither mode defeats traffic analysis. |
| 2026-09-04 | M-05 | Fixed for v3 adapters | `packetconn.go`, `securestream.go`, `traffic_shaper.go` | UDP integration, concurrent rekey, and full race suite | Ordered atomic MessageTransport behavior remains a caller obligation. |
| 2026-09-04 | L-01 | Mitigated in v3 paths | strict decoders, source binding, pending validation, and terminal stream cleanup | decoder/address/record fuzz smoke and `go vet ./...` | A fresh independent protocol review is still required before security approval. |

Verification for this v3 status: `go test ./...`,
`go test -shuffle=on -count=3 ./...`, `go test -cover ./...`,
`go test -race ./...`, `go vet ./...`, `go mod verify`, the v3 benchmark suite,
and one-second smoke runs for every v3 fuzz target passed on 2026-09-04.

## 11. v3 State and Allocation Follow-up (2026-09-05)

This entry records follow-up work without changing the independent deployment
review requirement above.

| Area | State | Verification | Residual risk |
|---|---|---|---|
| Finished delivery | Finished/session material is held only in the current handshake connection and cleared on success or failure; a failed carrier requires a fresh handshake | `TestV3HandlerWaitsForClientFinished`, `go test -race ./...` | No cross-carrier resume or persistent pending state is provided. |
| Decoder allocations | Fixed field table, specialized HelloRetry parser, and exact record byte fast paths remove map/setup allocations while preserving ownership and exact-wire behavior | v3 allocation benchmarks, ownership regression, full tests | Returned public message objects still require copies for exact-wire ownership and secret-clearing boundaries. |
| Codec differential checks | External-package independent HelloRetry and Address encoders compare byte-for-byte with v3 | `v3_interop_test.go`, `go test ./...` | Full independent handshake, KDF, record, and rekey interoperability is still required. |

The carrier and session lifecycle follow-up also covers context cancellation of
the dispatcher and application handler, session-local read/write deadlines,
post-close read semantics, and connection-local Finished state cleanup. These
behaviors are covered by `v3_udp_dispatch_test.go` and
`v3_integration_test.go`.

The measured v3 benchmark baseline after this follow-up is:

| Benchmark | Result |
|---|---|
| `BenchmarkV3CanonicalHelloRetryDecode` | 1 alloc, 128 B/op |
| `BenchmarkV3ClientInitDecode` | 4 allocs, 336 B/op |
| `BenchmarkV3FinishedDecode` | 2 allocs, 112 B/op |
| `BenchmarkV3RecordSeal` | 2 allocs, about 2304 B/op |
| `BenchmarkV3RecordOpen` | 3 allocs, about 2240 B/op |

## 12. v3 Scope Correction (2026-09-05)

The historical findings above considered deployment-wide replay protection and
cross-instance recovery as possible approval requirements. They are not part of
the EWP/v3 protocol contract. EWP/v3 is a connection-oriented protocol for a
single running endpoint:

- retry cookies are stateless, while replay, admission, and one-time prekey
  claim state is bounded in process memory;
- Finished state belongs to the current carrier and is discarded when that
  carrier closes;
- a carrier or process restart requires a new complete handshake; and
- the protocol defines no persistent store, shared cache, resume handoff, or
  database integration.

The first half of this document remains the historical audit record. The v3
implementation and its current documentation must be evaluated against this
reduced scope rather than against deployment features outside the protocol.
